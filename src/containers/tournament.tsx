...
import {
  getTournamentFromDimension,
  getUser,
} from '../actions/dimensions/dimensions';
...
function SetupTournament({
  ...
  useEffect(() => {
    if (type === 'general') {
      ...
      if (tournament.id === '') {
        getTournamentFromDimension(dimensionID, tournamentID)
          .then((res) => {
    ...
    if (tryAndLogin && user.loggedIn) {
      getUser(dimensionID, user.username).then((dim_user) => {
        if (dim_user) {
          ...
          if (cookie) {
            verifyToken(dimensionID, cookie)
              .then(() => {})
              .catch(() => {
